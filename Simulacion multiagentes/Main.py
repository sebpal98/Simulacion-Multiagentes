...
=======
    ...
    def check_collision(self):
        for traffict_light in arr_traffic_Lights:
            if self.rect.colliderect(traffict_light.collider_rect) and traffict_light.is_red():
                return True
        return False
...
=======
    ...
    def __init__(self, x_light, y_light, light_size, x_collider, y_collider, collider_size, color1):
        self.color = color1
        self.light_rect = pygame.Rect(x_light, y_light, light_size[0], light_size[1])
        self.collider_rect = pygame.Rect(x_collider, y_collider, collider_size[0], collider_size[1])
    ...
    for i in range(len(carros)):
        for j in range(i + 1, len(carros)):
            ...
            if carro1.check_collision_with_carro(carro2):
                return True  # Detener movimiento si hay colisión
    return False
...
def arrows():
    ...
    for y in range(80, window_width, 360):
        ...
        pygame.draw.polygon(window, WHITE, (
            [y, 240 + constant_roads], [y - 5, 230 + constant_roads], [y + 15, 240 + constant_roads],
            [y - 5, 250 + constant_roads]))
        # Fibujar las felchas izquierda
        pygame.draw.rect(window, WHITE, (y, 168 + constant_roads, 30, 5))
        pygame.draw.polygon(window, WHITE, (
            [y, 170 + constant_roads], [y + 5, 160 + constant_roads], [y - 15, 170 + constant_roads],
            [y + 5, 180 + constant_roads]))
    ...
    for y in range(40, window_height, 340):
        ...
        pygame.draw.polygon(window, WHITE, (
            [170 + constant_roads, y + 30], [180 + constant_roads, y + 25], [170 + constant_roads, y + 50],
            [160 + constant_roads, y + 25]))
        # Verticales de abajo abajo hacia arriba
        pygame.draw.rect(window, WHITE, (228 + constant_roads, y + 30, 5, 30))
        pygame.draw.polygon(window, WHITE, (
            [230 + constant_roads, y + 30], [240 + constant_roads, y + 35], [230 + constant_roads, y + 15],
            [220 + constant_roads, y + 35]))

        # Dibujar las flechas Horizontales
        pygame.draw.rect(window, WHITE, (168 + constant_roads * 2, y, 5, 30))
        pygame.draw.polygon(window, WHITE, (
            [170 + constant_roads * 2, y + 30], [180 + constant_roads * 2, y + 25], [170 + constant_roads * 2, y + 50],
            [160 + constant_roads * 2, y + 25]))
        # Verticales de abajo abajo hacia arriba
        pygame.draw.rect(window, WHITE, (228 + constant_roads * 2, y + 30, 5, 30))
        pygame.draw.polygon(window, WHITE, (
            [230 + constant_roads * 2, y + 30], [240 + constant_roads * 2, y + 35], [230 + constant_roads * 2, y + 15],
            [220 + constant_roads * 2, y + 35]))
...
<<<<<<< Updated upstream
        ...
        clock.tick(60)
=======
            if check_collision_between_cars(cars):  # Coliciones entre carritos
                car.has_collision = True
            if car.check_collision():  # Checa las coliciones
                car.has_collision = True
